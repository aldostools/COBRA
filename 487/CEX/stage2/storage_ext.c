...
/*
static uint8_t encrypted_image_keys[16] =
{
...
static unsigned int real_disctype; 		/* Real disc in the drive */
static unsigned int effective_disctype; 	/* The type of disc we want it to be, and the one faked in storage event. */
static unsigned int fake_disctype; 		/* If no zero, get device type command will fake disc type to this. */

static uint32_t cd_sector_size = 2352; // -- AV: cd sector size

//////////////////// READ & SAVE FILE /////////////////////

static size_t read_file_at_offset(const char *path, void *buf, size_t size, uint64_t offset)
{
	int fd;
...
static INLINE int process_read_iso_cmd(ReadIsoCmd *cmd)
{
	...
	if (ret == 0)
	{
		if (cache)
		{
			discfile->cached_sector = kalloc(2048);
...
int process_read_disc_cmd(ReadDiscCmd *cmd)
{
	...
	ret = page_allocate_auto(NULL, 4096, &dma);
...
int process_proxy_cmd(uint64_t command, process_t process, uint8_t *buf, uint64_t offset, uint32_t size)
{
	...
	else
	{
		...
		{
			kfree(discfile_proxy->cached_sector);
		...
		if (cache)
		{
			discfile_proxy->cached_sector = kalloc(2048);
...
static int process_get_psx_video_mode(void)
{
	...
	if (effective_disctype == DEVICE_TYPE_PSX_CD)
	{
		...
		page_allocate_auto(NULL, _4KB_, (void **)&dma);
...
int is_psx(int check_ps2)
{
	...
	if (page_allocate_auto(NULL, 2048, (void **)&buf) == 0)
	{
